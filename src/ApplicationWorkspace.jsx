...
export function ApplicationWorkspace({ data, openModal, refresh, notify, navigate, onOpenTable }) {
  ...
  return <div className="application-workspace">
    ...
    {application ? <article className="application-dossier">
      ...
      {tasks.length ? <section className="dossier-next-step"><div><span className="section-kicker">Next step for this application</span><strong>{tasks[0].title}</strong><small>{fullDate(tasks[0].dueDate)}</small></div><button className="text-action" type="button" onClick={() => openModal({ type: "task", task: tasks[0] })}>Open task<ArrowRight size={19} /></button></section> : null}
